...
complex<T>::complex (T real, T img)
    : mReal (real), mImg (img), mNorm (my::square::square ({ real, img })),
      mMagnitude (sqrt (mNorm))
{
}

template <typename T>
complex<T>::complex (const complex<T> &cmplex)
    : complex (cmplex.mReal, cmplex.mImg)
{
}
...
template <typename T>
complex<T> &
complex<T>::operator= (const float &num)
{
  this->mReal = num;
  this->mImg = num;
  return *this;
  ...
  if (rhs.norm () == 0)
    {
      throw std::runtime_error (
          "Can't divided with complex number having magnitude Zero\n");
    }
  auto t = lhs;
  t *= rhs.conj ();
  t /= rhs.mag ();
