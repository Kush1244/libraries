...
namespace my
{
template <typename numeric_type = float> class complex
{
  private:
    numeric_type mReal;
    numeric_type mImg;
...
template <typename T> complex<T>::complex() : complex(0.0, 0.0)
{
}

template <typename T> complex<T>::complex(T all_equal) : complex(all_equal, all_equal)
{
}

template <typename T>
complex<T>::complex(T real, T img)
    : mReal(real), mImg(img), mNorm(my::square::square({real, img})), mMagnitude(sqrt(mNorm))
{
}

template <typename T> complex<T>::complex(const complex<T> &cmplex) : complex(cmplex.mReal, cmplex.mImg)
{
}
...
template <typename T> inline T &complex<T>::img()
{
    return mImg;
}

template <typename T> T complex<T>::mag()
{
    return mMagnitude;
}

template <typename T> inline complex<T> complex<T>::conj()
{
    return complex(this->mReal, -this->mImg);
}

template <typename T> inline T complex<T>::norm()
{
    return mNorm;
}

/* MEMBER OPERATOR */
template <typename T> bool complex<T>::operator==(const complex<T> &other)
{
...
template <typename T> complex<T> &complex<T>::operator=(const float &num)
{
    this->mReal = num;
    this->mImg = num;
    return *this;
...
template <typename T> complex<T> operator+(complex<T> &lhs, complex<T> &rhs)
{
...
template <typename T> complex<T> operator-(complex<T> &lhs, complex<T> &rhs)
{
    return complex<T>(lhs.real() - rhs.real(), lhs.img() - rhs.img());
}
...
template <typename T> complex<T> operator*(complex<T> &lhs, T num)
{
...
template <typename T> complex<T> operator~(complex<T> &cn)
{
    return cn.conj();
}
